...
class UserAttendance extends ActiveRecord
{
    ...
    {
        $type = [
            1 => 'Regulárny pracovný čas',
            2 => 'Práce neschopný',
            3 => 'Návšteva lekára',
            4 => 'Neospravedlnené meškanie',
            5 => 'Ospravedlnené meškanie',
            6 => 'Nevyriešená absencia',
            7 => 'Dovolenka'
        ];
            ...
            select
                ...
                ifnull(
                    (
                        ...
                        where 
                            user_id=:uid and 
                            created_at between concat(ua.uaDate,' 00:00:00') and concat(ua.uaDate,' 23:59:59')
                    ),0
                ) as picCount
            from
                " . self::tableName() . " ua
    ...
    public function getMonthlyHoursByTypeByUser(int $userId, int $workType, ?int $month=null, ?int $year=null)
    {
        $year = $year ?? (new \DateTime('now'))->format('Y');
        $month = $month ?? $month = (new \DateTime('now'))->format('m');
        $sql = "
            select
                sum(ifnull(diffTime,0)) as diffTime
            from 
                userAttendance 
            where 
                uaDate like :date1 and 
                userId=:uid and
                uaType=:type1                
        ";
        $date1 = "$year-$month-%";
     ...
     * @throws \yii\db\Exception
     */
    public function getListForAdminByOptions(
        string $group='',
        string $startDate='',
        string $endDate='',
        ?int $userId=null
        ...
        $userFilter = '';
        if ($userId !== null) {
            $userFilter = " and ua.userId=$userId";
        }
        $groupFilter = "";
        if ('' !== $group && is_null($userId)) {
        ...
        $sql = "SELECT 
            ...
            FROM 
                userAttendance ua
            WHERE 
                1=1{$groupFilter}{$dateFilter}{$userFilter}
            ORDER BY 
                uaDate DESC";
