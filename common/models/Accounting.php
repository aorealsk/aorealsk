...
class Accounting
{
    ...
    public function buildInvoice(Invoice $invoice = null)
    {
        ...
        $res = preg_match('/[A-Z]{1,5}/', $this->invoiceData['cislo_faktury'], $znak);
        if ($res == 0) {
            throw new Exception('Wrong invoice sign!!!');
        }
        $this->invoiceData['znak'] = $znak[0];
        $this->invoiceData['rok'] = substr($this->invoiceData['cislo_faktury'], strlen($znak[0]), 4);
        $this->invoiceData['cislo'] = substr(
            $this->invoiceData['cislo_faktury'],
            strlen($znak[0]) + 4,
            strlen($this->invoiceData['cislo_faktury']) - strlen($znak[0]) - 4
        );
