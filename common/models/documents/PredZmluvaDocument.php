...
class PredZmluvaDocument extends Documents implements IContractDocument
{
    ...
    private function zmluvaPodpis()
    {
        ...
        $this->mpdf->y += ($cZoznamMajitelov)*40;
    ...
    private function zmluvaCenySluzby()
    {
        ...
        $items = Yii::$app->db->createCommand($sql)->bindValue(':num',$this->contractNumber)->queryAll();

        $cena = '';
        $provizia = '';

        foreach ($items as $item) {
            if ($item['zmluva_cena_typ'] == 'PREDAJ') {
                $cena .= $this->render('predzmluva-cena-predaj',[
                    'suma'   => number_format($item['cena'],2,"."," "),
                    'suma_slovom'    => ucfirst(HelperString::number2words($item['cena'])),
                ]);
                if ($item['provizia_typ'] == 'PERCENTO') {
                    $provizia .= $this->render('predzmluva-predaj-percento',['percento' => $item['percento']]);
                } else {
                    $provizia .= $this->render('predzmluva-predaj-pausal',[
                        'pausal'    => number_format($item['provizia'],2,"."," "),
                        'pausal_slovom' =>  ucfirst(HelperString::number2words($item['provizia']))
                    ]);
                }
            }

            if ($item['zmluva_cena_typ'] == 'PRENAJOM') {
                $cena .= $this->render('predzmluva-cena-prenajom',[
                    'suma'   =>  number_format($item['cena'],2,"."," "),
                    'suma_slovom'    => ucfirst(HelperString::number2words($item['cena'])),
                ]);
                if ($item['provizia_typ'] == 'PERCENTO') {
                    $provizia .= $this->render('predzmluva-prenajom-percento',['percento' => $item['percento']]);
                } else {
        ...
        $sql = "
            select 
                ...
                ifnull(c.platnost_od,'') as platnost_od,
                ifnull(c.platnost_do,'') as platnost_do,
                IF(c.platnost_od IS NULL,c.zmluva_platnost_mes,TIMESTAMPDIFF(MONTH,c.platnost_od,c.platnost_do)) as platnost_mesiac,
                IFNULL(c.zlava,0) as zlava,
                zs.pravne_sluzby,
        ...
        $item = Yii::$app->db->createCommand($sql)->bindValue(':num',$this->contractNumber)->queryOne();

        $sluzby = [];
        if (isset($item['pravne_sluzby']) && $item['pravne_sluzby'] == 1) {
            $sluzby[] = "<li>právne služby</li>";
        }
        if (isset($item['reklama']) && $item['reklama'] == 1) {
            $sluzby[] = "<li>náklady na reklamu</li>";
        }
        if (isset($item['cestovne']) && $item['cestovne'] == 1) {
            $sluzby[] = "<li>cestovné na obhliadky</li>";
        }
        if (isset($item['spravne_popl']) && $item['spravne_popl'] == 1) {
            $sluzby[] = "<li>správne poplatky (napr. na Správe katastra)</li>";
        }
        if (isset($item['sluzby_ine']) && $item['sluzby_ine'] == 1) {
            $sluzby[] = "<li>iné: {$item['sluzby_ine_popis']}</li>";
        }

        $dobaZmluvy = 'NEURČITÚ';
        $dobaZmluvyUrcita = '';
        if (isset($item['platnost_zmluvy']) && $item['platnost_zmluvy'] == 2) {
            $dobaZmluvy = 'URČITÚ';
            $dobaZmluvyUrcita = $this->render('predzmluva-doba-urcita',[
                'mesiace'   => $item['platnost_mesiac'],
                'mesiace_slovom' => HelperString::number2words($item['platnost_mesiac'])
            ]);
        }
        ...
        return $this->render('predzmluva-ceny-sluzby',[
            ...
            'zlava'         =>  "...".$item['zlava']."...",
            'cas_poskytnutia_sluzieb'   => $item['typ_zmluvy'] == 'standard' ? 'NEVÝHRADNOM' : 'VÝHRADNOM',
