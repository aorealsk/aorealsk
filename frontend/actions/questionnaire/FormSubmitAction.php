...
use common\models\client\ClientCompanyOwner;
...
class FormSubmitAction extends Action
{
    public function run()
    {
        if (Yii::$app->request->isPost) {
            ...
            $lvNumber = Yii::$app->request->post('lv_number');

            $client = new Client();
            $client->save();

            if (isset($_FILES['SpravcaFiles'])) {
            ...
            if (isset($_FILES['SpolocnikFiles'])) {
            ...
            if (isset($_FILES['KonateliaFiles'])) {
            ...
            $clientLv = new ClientCompanyLv();
            $clientLv->client_id = $client->id;
            $clientLv->lv_number = $lvNumber;
            $clientLv->save();

            $owner = new ClientCompanyOwner();
            $owner->client_id = $client->id;
            $owner->client_lv_number_id = $clientLv->id;
        ...
        Yii::$app->mailer
            ->compose('questionnaire-completion-html.php', [
                'clientReq' => $clientRequestData,
                'company' => $clientCompany,
                'spolocnik' => $spolocnikData,
                'konatel' => $konateliaData,
                'business' => $clientBusinessSubjectData,
                'zivnost' => $clientZivnost,
                'spravcaVkladu' => $spravcaVkladuData,
                'owner' => $ownerData
            ])->setFrom('company@aoreal.sk')
            ->setTo(Yii::$app->user->identity->email)
            ->send();

        return $this->controller->redirect('/questionnaire/thank-you');
    }
