...
$menuItems = [
    ['label' => 'O nás', 'url' => '#', 'submenu' => [
        ['label' => 'Náš tím', 'url' => '/makler-szabo-balazs'],
        ['label' => 'Kariéra', 'url' => '/kariera'],
        ['label' => 'Referencie', 'url' => '/referencie'],
        ['label' => 'Partneri', 'url' => '/partneri'],
        ['label' => 'Media', 'url' => '/media'],
    ]],
    ['label' => 'Nehnuteľnosti', 'url' => '#', 'submenu' => [
        ['label' => 'Exkluzívne ponuky', 'url' => '/exkluzivne-ponuky'],
        ['label' => 'Najnovšie ponuky', 'url' => '/najnovsie-ponuky'],
        ['label' => 'Všetky ponuky', 'url' => '/nehnutelnosti'],
        //array('label' => 'Hľadám', 'url' => '/hladam'),
        ['label' => 'PONÚKNITE NÁM!', 'url' => '/ponukam']
    ]],
    ['label' => 'Finančné poradenstvo', 'url' => '/financne-poradenstvo',
        'submenu'   => [
                ['label' => 'Finančný dotazník', 'url' => '/financny-dotaznik'],
        ]
    ],
    [
        'label' => 'All-inclusive service', 'url' => '/all-inclusive-service',
        'submenu' => [
            ['label' => 'Založenie firmy','url' => '/dotaznik-zalozenie-sro'],
            ['label' => 'Účtovníctvo', 'url' => '/uctovnictvo'],
            ['label' => 'Dual prax', 'url' => '/dual'],
        ]
    ],
...
$footerMenuItems = [
    ['label' => 'Reklamačný poriadok', 'url' => '/reklamacny-poriadok'],
    ['label' => 'Etický kódex', 'url' => '/eticky-kodex'],
    ['label' => 'GDPR', 'url' => '/ochrana-osobnych-udajov'],
    ['label' => 'Cenník', 'url' => '/cennik'],
];
...
<body data-rsssl=1 class="<?= (isset($this->params['class_body']) ? $this->params['class_body'] : 'home blog home-container'); ?>">
    <?php $this->beginBody(); ?>
	<div id="main-wrapper">
                ...
                <div class="container-fluid">
                    <div class="menu-panel__floating d-block position-absolute mx-auto w-100 text-right return-no-close">
                        <div class="menu-panel__floating-number d-inline-block align-middle text-white">
                            <a class="mobile-phone" href="tel:<?= $aoreal->phone_link; ?>">
                                <?= $aoreal->phone; ?>
                            </a>
                        </div>
                        <div class="menu-panel__floating-social d-inline-block align-middle ml-3">
                            <?php if (!empty($aoreal->facebook)) : ?>
                                <a href="<?= $aoreal->facebook ?>"
                                   target="_blank"
                                   class="ai-font-facebook d-inline-block mr-2 text-decoration-none align-middle text-white text-hover-gold">
                                </a>
                            <?php endif; ?>
                            <?php if (!empty($aoreal->twitter)) : ?>
                                <a href="<?= $aoreal->twitter ?>"
                                   target="_blank" class="ai-font-twitter d-inline-block mx-2 text-decoration-none align-middle text-white text-hover-gold">
                                </a>
                            <?php endif; ?>
                            <?php if (!empty($aoreal->instagram)) : ?>
                                <a href="<?= $aoreal->instagram ?>"
                                   target="_blank"
                                   class="ai-font-instagram d-inline-block mx-2 text-decoration-none align-middle text-white text-hover-gold">
                                </a>
                            <?php endif; ?>
                            <?php if (!empty($aoreal->linkedin)) : ?>
                                <a href="<?= $aoreal->linkedin ?>"
                                   target="_blank"
                                   class="ai-font-linkedin d-inline-block mx-2 text-decoration-none align-middle text-white text-hover-gold">
                                </a>
                            <?php endif; ?>
                            <?php if (!empty($aoreal->email)) : ?>
                                <a href="mailto:<?= $aoreal->encode_email($aoreal->email) ?>"
                                   class="ai-font-envelope-f d-inline-block ml-2 text-decoration-none align-middle text-white text-hover-gold">
                                </a>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="menu-panel__left d-inline-block position-absolute">
                        ...
                        <div class="menu-panel__logo d-inline-block position-absolute">
                            <a href="<?= Yii::$app->urlManager->createAbsoluteUrl(['/']); ?>"
                               class=" alt-icon-logo-mono text-white text-hover-gold text-decoration-none">
                            </a>
                        </div>
                    </div>
                    <div class="menu-panel__right d-inline-block align-middle w-100 position-absolute pr-3 pr-md-10 text-center">
                        <div class="menu-popup-menu-container return-no-close">
                            <?= $aoreal->getMainMenu($menuItems, 'menu-panel'); ?>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-xs-9 col-md-3 header__logo float-md-none d-md-inline-block align-middle">
                        <!--					<a href="<?//=esc_url( home_url() )?>" class="header__logo--link altman-font d-inline-block position-relative"><span class="hide"><?= $aoreal->company_name; ?></span></a>-->
                        <a href="<?= Yii::$app->urlManager->createAbsoluteUrl(['/']); ?>">
                            <img src="/images/logo-web-aoreal_white.png" class="img-responsive" alt="AO Real logo">
                        </a>
                    </div>
                    <div class="col-md-6 header__nav float-md-none d-none d-lg-inline-block align-middle" >
                        <nav class="text-center">
                            <div class="menu-inner-pages-main-menu-container">
                                <?= $aoreal->getMainMenu($menuItems, 'navigation'); ?>
                            </div>
                        </nav>
                    </div>
                    <div class="header__details float-md-none d-none align-middle text-right">
                        <div class="header__details--number d-inline-block align-middle text-white">
                            <a
                                    class="mobile-phone"
                                    href="tel:<?= $aoreal->phone_link; ?>"
                            >
                                <?= str_replace('+421 ','+421<span class="clear"></span>',$aoreal->phone); ?>
                            </a>
                        </div>
                        <div class="header__details--social d-inline-block align-middle ml-3">
                            <a
                                    href="<?= Yii::$app->urlManager->createAbsoluteUrl(['/kontakt']); ?>"
                                    class="ai-font-envelope-f d-inline-block text-decoration-none align-middle text-white text-hover-gold">
                            </a>
                        </div>
                    </div>
                ...
                <div class="row visible-onscroll">
                    <div class="col-lg-12">
                        <ul class="exclusive-menu">
                            <li>
                                <button
                                        class="btn btn-outline btn-estate-buy"
                                        type="button"
                                        data-filter-type="buy">
                                    <?= Aoreal::trans('Hľadám'); ?>
                                </button>
                            </li>
                            <li>
                                <button
                                        class="btn btn-primary btn-estate-offer"
                                        type="button"
                                        data-filter-type="offer">
                                    <a href="<?= Yii::$app->urlManager->createAbsoluteUrl(['/ponukam']); ?>">
                                        <?= Aoreal::trans('Ponúkam'); ?>
                                    </a>
                                </button>
                            </li>
                        </ul>
			...
			<div class="container footer__details position-relative">
                ...
                <div class="row py-3 py-md-6 d-none d-md-block">
                    <div
                            ...
                            data-aios-animation-offset="0.65">
                        <a href="" class="back-to-top d-inline-block position-relative py-2 px-3 raleway text-center text-uppercase">
                            <span class="back-to-top__text d-inline-block position-relative">
                                <?= Aoreal::trans('Späť na začiatok'); ?>
                                <em class="ai-font-arrow-b-u d-inline-block text-white"></em>
                            </span>
                        </a>
                    </div>
                </div>
                <div class="row pt-5">
                    <div
                            class="col-md-5 footer__logo text-right"
                            data-aios-reveal="true"
                            data-aios-animation="fadeInUp"
                            data-aios-animation-delay="0s"
                            data-aios-animation-reset="false"
                            data-aios-animation-offset="0.2"
                    >
                        <a href="<?= Yii::$app->urlManager->createAbsoluteUrl(['/']); ?>">
                            <img class="img-responsive" src="/images/logo-web-aoreal_white.png" alt="AO Real logo">
                            <span class="hide"><?= $aoreal->company_name; ?></span>
                        </a>
                    ...
                    <div
                            class="col-md-4 footer__list mt-5 mt-md-0"
                            data-aios-reveal="true"
                            data-aios-animation="fadeInUp"
                        ...
                        <ul class="footer__list--communities d-block">
                            <li>
                                <a href="<?= Yii::$app->urlManager->createAbsoluteUrl(['/exkluzivne-ponuky']); ?>">
                                    Exkluzívne ponuky
                                </a>
                            </li>
                            <li>
                                <a href="<?= Yii::$app->urlManager->createAbsoluteUrl(['/najnovsie-ponuky']); ?>">
                                    Najnovšie ponuky
                                </a>
                            </li>
                            <li><a href="<?= Yii::$app->urlManager->createAbsoluteUrl(['/nehnutelnosti']); ?>">
                                    Všetky ponuky
                                </a>
                            </li>
                            <?php foreach ($footerMenuItems as $footerMenuItem) : ?>
                                <li>
                                    <a href="<?= Yii::$app->urlManager->createAbsoluteUrl([$footerMenuItem['url']])?>">
					...
					<div class="col-md-3 footer__secondary pt-4 pt-md-0" data-aios-reveal="true" data-aios-animation="fadeInUp" data-aios-animation-delay="0.2s" data-aios-animation-reset="false" data-aios-animation-offset="0.2">
                        <a class="qrcode" href="<?= Yii::$app->urlManager->createAbsoluteUrl(['/kontakt']); ?>"><img src="/images/qrcode-aoreal.png" class="img-responsive" alt="<?= $aoreal->company_name; ?>"></a>
                    </div>
				</div>
			</div>
			<div class="footer_copyright container position-relative py-6 text-white">
				<div class="row">
					<div class="col-md-5 text-left">
						Webstránku vytvorila firma <a href="<?= $aoreal->copyright['url']; ?>" target="_blank" class="text-white text-hover-gold text-decoration-none"><strong><?= $aoreal->copyright['name']; ?></strong></a>
					</div>
					<div class="col-md-7 text-right">&copy; <?= (date('Y') > 2019 ? '2019-'.date('Y') : '2019').' '.$aoreal->company_name; ?> | Všetky práva vyhradené.</div>
				</div>
			</div>
			<div class="container">
...
<div class="sidebars-left">
    <button class="btn-search" title="<?= Aoreal::trans('Vyhľadávanie'); ?>" data-filter-type="none">
        <i class="fa fa-search"></i>
        <span><?= Aoreal::trans('Vyhľadávanie'); ?></span>
    </button>
    <div class="filter-block position-fixed animated" style="display: none;">
        <div class="filter-panel__left d-inline-block align-middle w-100 position-absolute pr-3 pr-md-10 text-center">
            ...
            <?php
            if (
                    !isset($this->params['class_body']) ||
                    (isset($this->params['class_body']) && $this->params['class_body'] != 'property-details')
            ) {
            ...
            <div class="citation">
                <blockquote>
                    Találd meg az otthonodat (...). Lehet, hogy nem a múltban találod meg, de találd meg,
                    és akkor soha többé nem leszel elveszett.</blockquote>
                <p>Pierce Brown</p>
            </div>
        </div>
...
<div class="super-sidebar">
            ...
            <ul>
                <li class="sb-facebook">
                    <a data-share="facebook">
                        ...
                        <div class="sb-label"><?= Aoreal::trans('Zdieľať na Facebooku'); ?></div>
                    </a>
                </li>
                <li class="sb-twitter">
                    <a data-share="twitter">
                        <div class="sb-icon fa fa-twitter"></div>
                        <div class="sb-label"><?= Aoreal::trans('Zdieľať na Twittere'); ?></div>
                    </a>
                </li>
                <li class="sb-google-plus">
                    <a data-share="google-plus">
                        <div class="sb-icon fa fa-google-plus"></div>
                        <div class="sb-label"><?= Aoreal::trans('Zdieľať na Google+'); ?></div>
                    </a>
                </li>
                <li class="sb-linkedin">
                    <a data-share="linkedin">
                        <div class="sb-icon fa fa-linkedin"></div>
                        <div class="sb-label"><?= Aoreal::trans('Zdieľať na LinkedIn'); ?></div>
                    </a>
                </li>
                <li class="sb-pinterest">
                    <a data-share="pinterest">
                        <div class="sb-icon fa fa-pinterest"></div>
                        <div class="sb-label"><?= Aoreal::trans('Zdieľať na Pinterest'); ?></div>
                    </a>
                </li><?php /*
                <li class="sb-reddit">
                    <a data-share="reddit">
                        <div class="sb-icon fa fa-reddit"></div>
                        <div class="sb-label"><?= Aoreal::trans('Zdieľať na Reddit'); ?></div>
                    </a>
                </li>
                <li class="sb-tumblr">
                    <a data-share="tumblr">
                        <div class="sb-icon fa fa-tumblr"></div>
                        <div class="sb-label"><?= Aoreal::trans('Zdieľať na Tumblr'); ?></div>
                    </a>
                </li>*/ ?>
            </ul>
        </li>
			...
			<ul>
				<li class="sb-about sb-space">
					<a href="#sidebar-about" target="_self">
						...
						<div class="sb-label"><?= Aoreal::trans('O nás'); ?></div>
					</a>
				</li>
				<li class="sb-contact">
					<a href="#sidebar-contact" target="_self">
						<div class="sb-icon fa fa-envelope"></div>
						<div class="sb-label"><?= Aoreal::trans('Kontaktujte nás'); ?></div>
					</a>
				</li>
				<li class="sb-newsletter">
					<a href="#sidebar-newsletter" target="_self">
						<div class="sb-icon fa fa-newspaper-o"></div>
						<div class="sb-label"><?= Aoreal::trans('Odber noviniek'); ?></div>
					</a>
				</li>
				<?php if ($aoreal->facebook) { ?>
				<li class="sb-facebook sb-space">
					<a href="<?= $aoreal->facebook; ?>" target="_blank">
						<div class="sb-icon fa fa-facebook"></div>
						<div class="sb-label"><?= Aoreal::trans('Facebook'); ?></div>
					</a>
				</li>
				<?php } ?>
				<?php if ($aoreal->instagram) { ?>
				<li class="sb-instagram">
					<a href="<?= $aoreal->instagram; ?>" target="_blank">
						<div class="sb-icon fa fa-instagram"></div>
						<div class="sb-label"><?= Aoreal::trans('Instagram'); ?></div>
					</a>
				</li>
				<?php } ?>
				<?php if ($aoreal->linkedin) { ?>
				<li class="sb-linkedin">
					<a href="<?= $aoreal->linkedin; ?>" target="_blank">
						<div class="sb-icon fa fa-linkedin"></div>
						<div class="sb-label"><?= Aoreal::trans('LinkedIn'); ?></div>
					</a>
				</li>
				<?php } ?>
				<?php if ($aoreal->twitter) { ?>
				<li class="sb-linkedin">
					<a href="<?= $aoreal->twitter; ?>" target="_blank">
						<div class="sb-icon fa fa-twitter"></div>
						<div class="sb-label"><?= Aoreal::trans('Twitter'); ?></div>
					</a>
				</li>
				<?php } ?>
			</ul>
    ...
    <div class="sb-window">
        ...
        <div id="sidebar-about" class="sb-panel">
            <div class="sb-head">
                <div class="sb-title"><?= Aoreal::trans('Pár slov o nás'); ?></div>
            ...
            <div class="sb-body">
                <div class="sb-clearfix">
                    <div class="sb-about-logo">
                        <img
                                src="/images/logo-web-aoreal_brown_small.png"
                                alt="<?= $aoreal->company_name; ?>"
                                class="img-responsive"
                        >
                    </div>
                    <div class="sb-about-desc">
                        <p>ALPHA-OMEGA REAL & CONSULTING  S.R.O. VZNIKLA S CIEĽOM POSKYTOVAŤ KOMPLEXNÉ SLUŽBY NA TRHU
                            S NEHNUTEĽNOSŤAMI A KVALITNÉ FINANČNÉ PORADENSTVO PRE VŠETKÝCH SVOJICH KLIENTOV.</p>
                        <p><i>„Inovácie nezávisia od množstva peňazí, ktoré máte vo vývoji a výskume.
                                Keď Apple prišiel s počítačom Mac, firma IBM mala stokrát viac peňazí, ale v tých to
                                nie je. Inovácie sú o vašich ľuďoch, ako ich vediete a nakoľko vidíte do problému.“ —
                                Steve Jobs, americký podnikateľ a spoluzakladateľ spoločnosti Apple Inc. 1955 - 2011
                                O ľuďoch, O problémoch</i>
                        </p>
                    </div>
                </div>
                ...
                <table class="sb-about-info">
                    <tbody>
                        <tr>
                            <td><label><?= Aoreal::trans('Telefón'); ?>:</label></td>
                            <td><?= $aoreal->phone; ?></td>
                        </tr>
                        <tr>
                            <td><label><?= Aoreal::trans('E-mail'); ?>:</label></td>
                            <td><?= $aoreal->email; ?></td>
                        </tr>
                        <tr>
                            <td><label><?= Aoreal::trans('Adresa'); ?>:</label></td>
                            <td><?= $aoreal->address ?> <br> <?= $aoreal->postcode ?> <?= $aoreal->city; ?></td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div id="sidebar-contact" class="sb-panel">
            <div class="sb-head">
                <div class="sb-title">KONTAKTUJTE NÁS</div>
                <div class="sb-close"></div>
            </div>
            <div class="sb-body">
                ...
                <form class="sb-form" action="super-sidebar/php/contact.php" data-id="contact">
                    <label><?= Aoreal::trans('Celé meno'); ?>:</label>
                    <input type="text" name="name" required>
                    <label><?= Aoreal::trans('E-mailová adresa'); ?>:</label>
                    <input type="text" name="email" required>
                    <label><?= Aoreal::trans('Správa'); ?>:</label>
                    <textarea name="message" required></textarea>
                    <div class="contactform_allowpt">
                        <label for="contactform_allowpt"><input type="checkbox" id="contactform_allowpt" required>
                        Súhlasím so správou, spracovaním a uchovaním mojich osobných údajov.</label>
                    </div>
                    <div class="sb-submit">Odosielať</div>
        ...
        <div id="sidebar-newsletter" class="sb-panel">
            <div class="sb-head">
                <div class="sb-title"><?= Aoreal::trans('ODBER NOVINIEK'); ?></div>
                <div class="sb-close"></div>
            </div>
            <div class="sb-body">
                <p><i>Prihláste sa k odberu noviniek, ak chcete byť e-mailom vždy informovaný o výhodných ponukách.
                        Pre zrušenie kontaktujte nás.</i></p>
                <form class="sb-form" action="super-sidebar/php/newsletter.php" data-id="newsletter">
                    <div class="sb-form-group">
                        <div><label><?= Aoreal::trans('Váš e-mail'); ?>:</label></div>
                        <div><input type="text" name="email" required></div>
                        <div><div class="sb-submit"><?= Aoreal::trans('Prihlásiť'); ?></div></div>
                    </div>
                    <div class="sb-form-group">
    ...
    jQuery(document).ready(function() {
        jQuery(".super-sidebar").superSidebar({
            ...
            formData: {
                "contact": {
                    status: {
                        empty: "<?= Aoreal::trans('Vyplňte prosím všetky povinné polia.'); ?>",
                        badEmail: "<?= Aoreal::trans('Formát e-mailu je nesprávny.'); ?>",
                        working: "<?= Aoreal::trans('Posiela sa správa, čakajte prosím ...'); ?>",
                        success: "<?= Aoreal::trans('Správa bola úspešne odoslaná!'); ?>",
                        error: "<?= Aoreal::trans('Vyskytla sa chyba! Správa nebola odoslaná.'); ?>"
                    }
                },
                "newsletter": {
                    status: {
                        empty: "<?= Aoreal::trans('Zadajte prosím Vašu e-mailovú adresu.'); ?>",
                        badEmail: "<?= Aoreal::trans('Formát e-mailu je nesprávny.'); ?>",
                        working: "<?= Aoreal::trans('Prebieha registrácia, čakajte prosím ...'); ?>",
                        success: "<?= Aoreal::trans('Vaša registrácia bola úspešná!'); ?>",
                        error: "<?= Aoreal::trans('Vyskytla sa chyba! Prosím skúste to znova.'); ?>",
                        consent: "<?= Aoreal::trans('Musíte súhlasiť so spracovaním a uchovávaním Vašich osobných údajov!'); ?>"
                    }
                }
            }
        });
