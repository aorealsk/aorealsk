...
$this->title = Yii::t('app','Zákaznícka zóna - registrácia');
...
<main class="site-signup" id="d1">
    ...
    <section id="contact-section-one">
        <div class="contactus-form" style="margin-top: 0px; width: 50%; margin-bottom: 50px;">
            <h2 class="raleway"><?= Yii::t('app','Vytvorenie nového účtu') ?></h2>
            <div role="form" class="wpcf7" id="wpcf7-f2130-o1" lang="en-US" dir="ltr">
                <div class="screen-reader-response"></div>
                <?php $form = ActiveForm::begin(['id' => 'signup-form']); ?>

                <?= $form->field($model, 'firstName')->label(Yii::t('app','Meno')) ?>
                <?= $form->field($model, 'lastName')->label(Yii::t('app','Priezvisko')) ?>
                <?= $form->field($model, 'email') ?>
                <?= $form->field($model, 'country')->label(Yii::t('app','Krajina'))->dropDownList(
                    $countries
                ) ?>
                <?= $form->field($model, 'phone')->label(Yii::t('app','Mobil')) ?>
                <?= $form->field($model, 'password')->label(Yii::t('app','Heslo'))->passwordInput() ?>
                <?= $form->field($model, 'passwordConfirm')->label(Yii::t('app','Potvrdiť heslo'))->passwordInput() ?>

                <div class="form-group mt-10">
                    <?= Html::submitButton(Yii::t('app','Registrovať'), ['class' => 'btn btn-primary', 'name' => 'login-button']) ?>
                </div>
                <?php ActiveForm::end(); ?>
            </div>
        </div>
