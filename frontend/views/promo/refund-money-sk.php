...
<div class="container mx-auto p-4 w-50">
    ...
    <h3 class="mt-5">Vážený/á <?= $guest->getFullName() ?>,</h3>
    ...
    <p class="mt-5">na svojom virtuálnom účte máte ešte
        <span id="bal"><b><?= number_format($guest->balance, 2) ?></b></span>
        &euro;. Čo chcete urobiť so zostatkom na účte?
    </p>
    <form method="post" role="form">

        <div class="row mt-5">
            <div class="col-8">Ponúkam na dobročinné účely príjemcovi tomboly</div>
            <div class="col-4">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="c-full" name="c_full">
                    <label class="form-check-label">celú sumu</label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="c-partial" name="c_part">
                    <label class="form-check-label">čiastočne</label>
                    <input
                        type="text"
                        id="charity-amount"
        ...
        <div class="row mt-5">
            <div class="col-8">Ponúkam to čašníkom ako prepitné</div>
            <div class="col-4">
                <div class="form-check">
        ...
        <div class="row mt-5">
            <div class="col-8">
                Pošlite mi to späť na môj účet
                <br>
                <br>
        ...
        <div class="row mt-5">
            <div class="col-12">
                <button type="button" class="btn btn-primary" id="send-frm">Poslať žiadost</button>
            </div>
        </div>
...
<script>
    ...
    $('#send-frm').on('click', function(){
        if ($('#c-partial').is(':checked') && $('#charity-amount').val() == ''){
            alert('Zadajte sumu daru!');
            return;
        }
        if ($('#t-partial').is(':checked') && $('#tip-amount').val() == ''){
            alert('Zadajte výšku prepitného!');
            return;
        }
        if ($('#b-partial').is(':checked') && $('#back-amount').val() == ''){
                alert('Zadajte sumu, ktorá sa má vrátiť!');
            return;
        }
        if ($('#b-partial').is(':checked') && $('#iban').val() == ''){
            alert('Zadajte číslo svojho bankového účtu!');
            return;
        }
        ...
        if (
            ...
            alert('Prosím, vyberte aspoň jednu možnosť!');
            return;
        }
        ...
        let iban = $('#iban').val();
        if (iban != '' && !isIban(iban)){
            alert('Zadajte platné číslo bankového účtu!');
            return;
        }
