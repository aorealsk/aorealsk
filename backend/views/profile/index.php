...
/** Null-safe helpers (additive) */
$identity = Yii::$app->user->identity; // common\models\User
$pic = '/img/default-avatar.png';
if (is_object($identity) && method_exists($identity, 'getProfilePicture')) {
    $tmpPic = $identity->getProfilePicture();
    if (!empty($tmpPic)) { $pic = $tmpPic; }
}
$displayName = null;
if (isset($agent) && is_object($agent) && method_exists($agent, 'getFullName')) {
    $displayName = $agent->getFullName();
}
if (!$displayName) {
    $first = $identity->name_first ?? '';
    $last  = $identity->name_last ?? '';
    $displayName = trim($first . ' ' . $last);
}
if (!$displayName) { $displayName = $identity->username ?? Yii::t('app','Nový používateľ'); }

$email = (isset($agent) && !empty($agent->email)) ? $agent->email
       : (!empty($identity->email) ? $identity->email
       : ((isset($userDetails) && !empty($userDetails->email)) ? $userDetails->email : '—'));

$phone = (isset($agent) && !empty($agent->phone)) ? $agent->phone
       : (!empty($identity->phone) ? $identity->phone
       : ((isset($userDetails) && !empty($userDetails->phone)) ? $userDetails->phone : '—'));

$fullAddress = '—';
if (isset($agent) && is_object($agent) && method_exists($agent, 'getFullAddress')) {
    $fullAddress = $agent->getFullAddress() ?: '—';
}

/** Prefill values for the edit form (falls back to identity if agent is missing) */
$valFirst = isset($agent) && !empty($agent->name_first) ? $agent->name_first : ($identity->name_first ?? '');
$valLast  = isset($agent) && !empty($agent->name_last)  ? $agent->name_last  : ($identity->name_last  ?? '');
$valEmail = isset($agent) && !empty($agent->email)      ? $agent->email      : ($identity->email      ?? '');
$valPhone = isset($agent) && !empty($agent->phone)      ? $agent->phone      : ($identity->phone      ?? '');
?>
       ...
       <div class="col-lg-4 col-xlg-3 col-md-5">
           <div class="card">
                   ...
                   <div class="row m-t-30">
                       <div class="col-md-12 text-center">
                           <img
                                   ...
                                   class="img-circle"
                                   width="150">
                           <h4 class="card-title m-t-30"><?= Html::encode($displayName) ?></h4>
               ...
               <div class="card-body"> <small class="text-muted"><?= Yii::t('app','Email'); ?></small>
                   <h6><?= Html::encode($email) ?></h6>
                   <small class="text-muted p-t-30 db"><?= Yii::t('app','Telefón') ?></small>
                   <h6><?= Html::encode($phone) ?></h6>
                   <small class="text-muted p-t-30 db"><?= Yii::t('app','Adresa') ?></small>
                   <h6><?= Html::encode($fullAddress) ?></h6>
                   <p class="text-muted p-t-30 db small"><?= Yii::t('app','Sociálne siete') ?></p>

                   <?php
                   if (!empty($userDetails) && $userDetails->facebook != '') {
                   ?>
                   <a class="btn btn-circle btn-secondary" href="<?= Html::encode($userDetails->facebook)?>" target="_blank"><i class="fab fa-facebook-f"></i></a>
                   <?php
                   }
                   if (!empty($userDetails) && $userDetails->linkedin != '') {
                   ?>
                   <a class="btn btn-circle btn-secondary" href="#"><i class="fab fa-linkedin"></i></a>
                   <?php
                   }
                   if (!empty($userDetails) && $userDetails->twitter != '') {
                   ?>
                   <a class="btn btn-circle btn-secondary" href="#"><i class="fab fa-twitter"></i></a>
                   <?php
                   }
                   if (!empty($userDetails) && $userDetails->youtube != '') {
                   ?>
                   <a class="btn btn-circle btn-secondary" href="#"><i class="fab fa-youtube"></i></a>
                   <?php
                   }
                   if (!empty($userDetails) && $userDetails->instagram != '') {
                   ?>
                           ...
                           <form class="form-horizontal" method="post">
                               ...
                               <div class="form-group">
                                   ...
                                   <div class="input-group">
                                       ...
                                       <input type="text" class="form-control" value="<?= Html::encode($valFirst) ?>" name="name_first">
                                   </div>
                               </div>
                               <div class="form-group">
                                   ...
                                   <div class="input-group">
                                       <span class="input-group-text"><i class="ti-user"></i></span>
                                       <input type="text" class="form-control" value="<?= Html::encode($valLast) ?>" name="name_last">
                                   </div>
                               </div>
                               <div class="form-group">
                                   ...
                                   <div class="input-group">
                                       ...
                                       <input type="email" class="form-control" name="email" value="<?= Html::encode($valEmail) ?>">
                                   </div>
                               </div>
                               <div class="form-group">
                                   ...
                                   <div class="input-group">
                                       ...
                                       <input type="text" class="form-control" name="phone" value="<?= Html::encode($valPhone) ?>">
                                   </div>
                               </div>
                               ...
                               <div class="form-group">
                                   ...
                                   <div class="input-group">
                                       ...
                                       <input type="text" name="facebook" class="form-control" value="<?= !is_null($userDetails) ? Html::encode($userDetails->facebook) : '' ?>">
                                   </div>
                               </div>
                               <div class="form-group">
                                   ...
                                   <div class="input-group">
                                       ...
                                       <input type="text" class="form-control" name="linkedin" value="<?= !is_null($userDetails) ? Html::encode($userDetails->linkedin) : '' ?>">
                                   </div>
                               </div>
                               <div class="form-group">
                                   ...
                                   <div class="input-group">
                                       ...
                                       <input type="text" class="form-control" name="twitter" value="<?= !is_null($userDetails) ? Html::encode($userDetails->twitter) : '' ?>">
                                   </div>
                               </div>
                               <div class="form-group">
                                   ...
                                   <div class="input-group">
                                       ...
                                       <input type="text" class="form-control" name="youtube" value="<?= !is_null($userDetails) ? Html::encode($userDetails->youtube) : '' ?>">
                                   </div>
                               </div>
                               <div class="form-group">
                                   ...
                                   <div class="input-group">
                                       ...
                                       <input type="text" class="form-control" name="instagram" value="<?= !is_null($userDetails) ? Html::encode($userDetails->instagram) : '' ?>">
                                   </div>
                               </div>
                               <div class="form-group">
