                ...
                <div class="card-body">
                    ...
                    <form method="post" role="form" class="form p-t-20">
                        ...
                        <div class="form-group row">
                            ...
                            <label class="form-label text-end col-sm-1">
                                <?= Yii::t('app','IP'); ?>
                            </label>
                            <div class="col-4">
                        ...
                        <div class="form-group row">
                            <label class="form-label text-end col-3">
                                <?= Yii::t('app', 'Príchod (prez.)') ?>
                            </label>
                            <div class="col-9">
                                <input type="time" class="form-control" id="inorigtime" value="<?= $item['inOrigTime'] ?>'">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="form-label text-end col-3">
                                <?= Yii::t('app', 'Odchod (prez.)') ?>
                            </label>
                            <div class="col-9">
                                <input type="time" class="form-control" id="outorigtime" value="<?= $item['outOrigTime'] ?>'">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="form-label text-end col-3">
                        ...
                        <div class="form-group row">
                            <label class="form-label text-end col-3"><?= Yii::t('app','Poznámky'); ?></label>
                            <div class="col-sm-9">
...
$deleteMessage = Yii::t('app','Naozaj chcete zmazať obrázok ');

$js = <<<JS
removeImage = function(i,s) {
    if (window.confirm("{$deleteMessage} " + s + "?")) {
        $.ajax({
            url: "/backoffice/user-attendance-admin/remove-picture",
            dataType: "json",
            data: { 
                imageid: i,
                {$csrf} 
...
$('#svatt').click(()=>{
    $.ajax({
            ...
            data: { 
                ...
                outip: $('#outip').val(),
                uanote: $('#uanote').val(),
                inorigtime: $('#inorigtime').val(),
                outorigtime: $('#outorigtime').val(),
                {$csrf} 
            },
            type: "post"
        })
