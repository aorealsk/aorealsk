...
class ContractsController extends Controller
{
    ...
    public function actionAjaxGetClients($q)
    {
        ...
        $sql = "SELECT 
                    c.id,
                    CONCAT(name_first,' ',name_last,' - ',ssn) AS klient
                FROM 
                    customer c
                WHERE 
                    name_last LIKE '%$q%'";
    ...
    public function actionAjaxGenDocs()
    {
        ...
        try {
            foreach ($docs as $item) {
                ...
                $doc->create();
                $result[$item['doc']][$item['poradie'] - 1] =
                    Yii::getAlias('@web') . "/docstore/" . $request['num'] . '/' . $doc->getFileName();
                unset($doc);
            }
        } catch (\Throwable $ex) {
            throw $ex;
    ...
    public function actionZmazLv()
    {
        Yii::$app
            ->db
            ->createCommand("update nehnut_dokumenty set status=0 where zmluva_cislo= :cislozmluvy")
            ->bindValue(':cislozmluvy', $_REQUEST['cislo'])
            ->execute();
        return json_encode([
            "success" => true,
            'item' => $this->renderPartial('majitelia-prazdne'),
        ]);
    }
    ...
    public function actionAjaxGetTownData()
    {
        ...
        return Yii::$app
            ->db
            ->createCommand("select id, nazov_obce as label, nazov_obce as `value` from mesto where nazov_obce like '%$term%'")
            ->queryAll();
    }
}
