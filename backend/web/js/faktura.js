...
$(document).on('select2:select','#get-odber',function(){
    ...
    const odber = JSON.parse($(this).val());

    $('#odberatel-name').val(odber.nazov);
    $('#odberatel-contactperson').val(odber.kontaktna_osoba);
    $('#odber-email').val(odber.email);
    $('#odber-phone').val(odber.phone);
    $('#odber-web').val(odber.web);
    $('#odberatel-address').val(odber.ulica);
    $('#odberatel-zip').val(odber.psc);
    $('#oberatel-country').val(odber.stat);
    $('#odberatel-ico').val(odber.ico);
    $('#odberatel-dic').val(odber.dic);
    $('#odberatel-icdph').val(odber.icdph);
    $('#doda-nazov').val(odber.dodacia_nazov);
    $('#doda-osoba').val(odber.dodacia_kontaktna_osoba);
    $('#doda-ulica').val(odber.dodacia_ulica);
    $('#dodacia-zip').val(odber.dodacia_psc);
    $('#dodacia-country').val(odber.dodacia_stat);

    $('#odberatel-town option').each(function(){
        let vl_str = $(this).val();
        if (vl_str.length !== 0) {
            let vl = JSON.parse(vl_str);
            let psc1 = vl.psc.replace(' ','');
            if ( psc1 === odber.psc.replace(' ','') && vl.nazov_obce === odber.mesto) {
